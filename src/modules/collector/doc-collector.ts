...
export class DocCollector implements ISourceCollector<DocItem> {
  ...
  private async listAllDocsInFolder(
    ...
    do {
      const response: any = await this.feishuClient!.request('GET', '/open-apis/drive/v1/files', {
        params: {
  ...
  private async getDocContentSummary(docToken: string, docType: string): Promise<string> {
    try {
      if (docType === 'docx') {
        // 获取文档的基本信息和前几段内容
        const response: any = await this.feishuClient!.request('GET', `/open-apis/docx/v1/documents/${docToken}/raw_content`);
